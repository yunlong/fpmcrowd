...
static int fpm_unix_conf_wp(struct fpm_worker_pool_s *wp)
{
	...
	zlog(ZLOG_STUFF, ZLOG_NOTICE, "home:%s user:%s uid:%d gid:%d", wp->user,wp->home,wp->set_uid,wp->set_gid );
	return 0;
}
...
int fpm_unix_init_main()
{
	...
	if (fpm_global_options.daemonize) {
		...
		switch (fork()) {
			...
			case 0 :
				zlog(ZLOG_STUFF, ZLOG_NOTICE, "master process %d started", getpid() ); 
				break;

			default :
			//	zlog(ZLOG_STUFF, ZLOG_NOTICE, "orig parent process %d is destoryed", getpid() ); 

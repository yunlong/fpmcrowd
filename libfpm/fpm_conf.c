...
static void *fpm_worker_pool_config_alloc()
{
	...
	zlog(ZLOG_STUFF, ZLOG_NOTICE, "orig parent process %d alloc worker pool OK", getpid() );

	wp->config = malloc(sizeof(struct fpm_worker_pool_config_s));

	if (!wp->config) return 0;
	
	zlog(ZLOG_STUFF, ZLOG_NOTICE, "orig parent process %d alloc worker pool config OK", getpid() );
...
int fpm_conf_write_pid()
{
	...
	if (fpm_global_options.pid_file) {
		...
		zlog(ZLOG_STUFF, ZLOG_NOTICE, "master process %d write pid %d to file %s OK", getpid(), fpm_globals.parent_pid, fpm_global_options.pid_file);
...
static int fpm_conf_post_process()
{
	...
	if (!fpm_global_options.error_log) {
		fpm_global_options.error_log = strdup(FPM_LOG_PATH);
	}
